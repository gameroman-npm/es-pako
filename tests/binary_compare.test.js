...
describe("Deflate vs canonical zlib snapshots (legacyHash)", () => {
  function testSample(pako_method, sample, options, filename) {
    ...
    const pako_result = pako_method(
      sample,
      Object.assign({ legacyHash: true }, options),
    );
    ...
    for (const windowBits of [15, 14, 13, 12, 11, 10, 9, 8]) {
      it(`windowBits ${windowBits}`, () => {
        testSample(
          ...
          sample,
          { windowBits },
          `deflate_windowBits=${windowBits}.bin`,
        );
      });
    }
...
describe("Deflate vs node.js zlib (default hash)", () => {
  ...
  describe("Deflate levels", () => {
    for (const level of [9, 8, 7, 6, 5, 4, 3, 2, 1, -1]) {
      it(`level ${level}`, () => {
        testNode(
          pako.deflate(sample, { level }),
          zlib.deflateSync(buf, { level }),
        );
      });
    }
  });
  ...
  describe("Deflate windowBits", () => {
    for (const windowBits of [15, 14, 13, 12, 11, 10, 9, 8]) {
      it(`windowBits ${windowBits}`, () => {
        testNode(
          pako.deflate(sample, { windowBits }),
          zlib.deflateSync(buf, { windowBits }),
        );
      });
    }
    it("windowBits -15 (implicit raw)", () => {
      testNode(
        pako.deflate(sample, { windowBits: -15 }),
        zlib.deflateRawSync(buf, { windowBits: 15 }),
      );
    });
  });
  ...
  describe("Deflate memLevel", () => {
    for (const memLevel of [9, 8, 7, 6, 5, 4, 3, 2, 1]) {
      it(`memLevel ${memLevel}`, () => {
        testNode(
          pako.deflate(sample, { memLevel }),
          zlib.deflateSync(buf, { memLevel }),
        );
      });
    }
  });
  ...
  describe("Deflate strategy", () => {
    it("Z_DEFAULT_STRATEGY", () => {
      testNode(
        pako.deflate(sample, { strategy: 0 }),
        zlib.deflateSync(buf, { strategy: 0 }),
      );
    });
    it("Z_FILTERED", () => {
      testNode(
        pako.deflate(sample, { strategy: 1 }),
        zlib.deflateSync(buf, { strategy: 1 }),
      );
    });
    it("Z_HUFFMAN_ONLY", () => {
      testNode(
        pako.deflate(sample, { strategy: 2 }),
        zlib.deflateSync(buf, { strategy: 2 }),
      );
    });
    it("Z_RLE", () => {
      testNode(
        pako.deflate(sample, { strategy: 3 }),
        zlib.deflateSync(buf, { strategy: 3 }),
      );
    });
    it("Z_FIXED", () => {
      testNode(
        pako.deflate(sample, { strategy: 4 }),
        zlib.deflateSync(buf, { strategy: 4 }),
      );
    });
  });
  ...
  describe("Deflate RAW", () => {
    for (const level of [4, 1]) {
      it(`level ${level}`, () => {
        testNode(
          pako.deflateRaw(sample, { level }),
          zlib.deflateRawSync(buf, { level }),
        );
      });
    }
  });
  ...
  describe("Deflate dictionary", () => {
    it("trivial dictionary", () => {
      const dict = Buffer.from("abcdefghijklmnoprstuvwxyz");
      testNode(
        pako.deflate(sample, { dictionary: dict }),
        zlib.deflateSync(buf, { dictionary: dict }),
      );
    });
    ...
    it("spdy dictionary", () => {
      ...
      testNode(
        pako.deflate(sample, { dictionary: spdyDict }),
        zlib.deflateSync(buf, { dictionary: spdyDict }),
      );
    });
  });
});
